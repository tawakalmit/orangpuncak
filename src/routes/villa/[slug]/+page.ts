import { getPlaceBySlug, getRelatedVillas, getNearbyWithFallback, getArticles } from '$lib/data';
import { error } from '@sveltejs/kit';
import type { PageLoad } from './$types';

export const load: PageLoad = async ({ params }) => {
	const place = await getPlaceBySlug('villa', params.slug);
	...
	const [related, nearbyVilla, nearbyWisata, nearbyKuliner, { articles }] = await Promise.all([
		getRelatedVillas(params.slug, 8),
