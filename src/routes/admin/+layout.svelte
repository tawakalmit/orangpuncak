<script lang="ts">
	...
	const nav = [
		...
		{ href: '/admin/places', label: 'Direktori' },
		{ href: '/admin/articles', label: 'Artikel' },
		{ href: '/admin/leads', label: 'Leads Iklan' }
	];
	...
	<div class="flex min-h-screen bg-cream">
		<aside class="hidden w-64 shrink-0 flex-col bg-brand text-white md:flex">
			<div class="border-b border-white/15 px-5 py-4">
				<a href="/admin" class="font-logo text-2xl font-bold">orang<span class="text-accent">puncak</span></a>
		...
		<div class="flex-1">
			...
			<div class="flex items-center justify-between bg-brand px-4 py-3 text-white md:hidden">
				<a href="/admin" class="font-logo text-xl font-bold">orang<span class="text-accent">puncak</span></a>
				<form method="POST" action="/admin/logout" use:enhance>
